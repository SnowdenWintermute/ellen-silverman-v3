...
const AddressManager = () => {
  ...
  const selectedShippingAddress = useSelector(
    (state) => state.cart.selectedShippingAddress
  );
  ...
  const [addressFromServerToBeConfirmed, setAddressFromServerToBeConfirmed] =
    useState({});
    ...
    try {
      ...
      if (!selectedShippingAddress) {
        if (addresses.data[0] !== null && addresses.data.length)
          dispatch(setSelectedShippingAddress(addresses.data[0]._id));
      }
  ...
  const handleSubmitNewAddress = async (e) => {
    ...
    try {
      ...
      if (newAddress.data && newAddress.data.city)
        setConfirmAddressModalOpen(true);
      if (!newAddress.data)
        toast.error(
          "Error validating address. Please double check all fields."
        );
      else if (!newAddress.data.city)
        toast.error(
          "Error validating address. Please double check all fields."
        );
    } catch (error) {
  ...
  const handleConfirmAddress = async () => {
    try {
      const newAddresses = await confirmNewAddress(
        addressFromServerToBeConfirmed._id,
        user.token
      );
      setConfirmedAddresses(newAddresses.data);
      dispatch(
        setSelectedShippingAddress(
          newAddresses.data[newAddresses.data.length - 1]._id
        )
      );
  ...
  const handleRemoveAddress = async (addressId) => {
    try {
      ...
      if (newAddressList.data[0])
        dispatch(setSelectedShippingAddress(newAddressList.data[0]._id));
  ...
  const handleSelectAddressChange = (e) =>
    dispatch(setSelectedShippingAddress(e.target.value));
