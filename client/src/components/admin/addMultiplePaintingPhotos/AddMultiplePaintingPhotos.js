...
const AddMultiplePaintingPhotos = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setProgress(0);
    try {
      const res = await uploadMultiplePaintingImages(
        formData.current,
        user.token,
        handleProgressEvent
      );
    ...
    <div className="page-frame">
      <MaterialPaperNarrow>
        <Grid container item xs={12}>
          <AdminFeatureHeader
            headerText={"Add Multiple Painting Images"}
            subHeaderText={"Image names must match painting names exactly"}
          />
          <Grid item xs={12}>
          ...
          <Grid item container spacing={1} xs={12}>
            {photos.map((photo, i) => (
              <ImagePreviewCard key={i} photo={photo} />
            ))}
          </Grid>
        </Grid>
