...
const AddPaintingsFromCSV = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await uploadPaintingCSVFormData(
        formData.current,
        user.token,
        handleProgressEvent
      );
      ...
      res.data.paintingsUpdated &&
        setPaintingsUpdated(res.data.paintingsUpdated);
    ...
    <div className="page-frame">
      <MaterialPaperNarrow>
        <Grid container item xs={12}>
          <AdminFeatureHeader
            headerText={"Add and Update Paintings from CSV"}
            subHeaderText={
              "NOTE: Any paintings in the selected .csv file will overwrite paintings in the database, except for their sold status"
            }
          />
          <Grid item xs={12}>
      ...
      <StandardDialog open={open} onClose={handleClose}>
        ...
        <AddedPaintingsResultsAccordion
          paintingsAdded={paintingsAdded}
          paintingsUpdated={paintingsUpdated}
          errors={errors}
        />
        <PrimaryButton
          title="OK"
